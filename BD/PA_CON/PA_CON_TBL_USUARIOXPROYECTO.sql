...
EXEC dbo.PA_CON_USUARIOXPROYECTO
...
*/
GO
CREATE PROCEDURE [dbo].[PA_CON_USUARIOXPROYECTO]
(
     ...
     @P_IDUSUARIO BIGINT = 0,			  /*PK Y FK DE LA TABLA USUARIO*/
     @P_IDPROYECTO BIGINT = 0,			  /*PK Y FK DE LA TABLA PROYECTO*/
	 @P_FECHA DATETIME,			          /*FECHA EN QUE EL USUARIO FUE AGREGADO*/
...
BEGIN
        ...
        BEGIN --INICIO-CUERPO PROCEDIMIENTO
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    SELECT @V_ERR_MENSAJE_USUARIO = 'ERROR AL EJECUTAR LA CONSULTA DE LA TABLA TBL_SIMP_USUARIOXPROYECTO';

				SELECT	@SET_SQLSTRING =
				N'SELECT [PK_FK_TBL_SIMP_USUARIO]
					  ,[PK_FK_TBL_SIMP_PROYECTO]
					  ,[FECHA]
				  FROM ' + @P_ESQUEMA + N'.[TBL_SIMP_USUARIOXPROYECTO]'
				...
				N'SELECT [PK_FK_TBL_SIMP_USUARIO]
					  ,[PK_FK_TBL_SIMP_PROYECTO]
					  ,[FECHA]
				  FROM ' + @P_ESQUEMA + N'.[TBL_SIMP_USUARIOXPROYECTO] WHERE  ([PK_FK_TBL_SIMP_USUARIO] =  @P_IDUSUARIO OR @P_IDUSUARIO = 0) 
				  AND ([PK_FK_TBL_SIMP_PROYECTO] = @P_IDPROYECTO OR @P_IDPROYECTO = 0)
				  AND ([FECHA] = @P_FECHA OR @P_FECHA = '''')'
    ...
    BEGIN CATCH
       ...
       SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, '@P_OPCION=', ISNULL(@P_OPCION, -1));
		...
		SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_FECHA=', '''', ISNULL(@P_FECHA, 'N/D'),'''');
