...
FECHA:          <2022/09/17>
DESCRIPCION:   <SELECCIONA ACTIVIDADES>
-------------------------------------------

=========================================COMPLETAR CON NUEVOS PARAMETROS============================================================================

EXEC dbo.PA_CON_ACTIVIDAD		@P_USUARIO = '',            -- varchar(20)
                                    @P_OPCION = 0,              -- smallint
                                    @P_ID = 0,					-- int
                                    @P_ESTADO = '',             -- char(1)
...
(
     ...
     @P_ID BIGINT = 0,								/*ID ACTIVIDAD */
     @P_IDFASE BIGINT = 0,						    /*FK FASE */
     @P_IDUSUARIO BIGINT = 0,						/*FK USUARIO */
     @P_IDESTADO BIGINT = 0,						/*FK ESTADO */
     @P_DESCRIPCION VARCHAR(250) = '',			    /*DESCRIPCION FASE*/
     @P_FECHA_INICIO DATETIME,			            /*FECHA INICIO ACTIVIDAD*/
	 @P_FECHA_ESTIMADA DATETIME,					/*FECHA ESTIMADA DE FINALIZACION DE LA ACTIVIDAD*/
     @P_FECHA_FINALIZACION DATETIME,				/*FECHA REAL DE FINALIZACION DE LA ACTIVIDAD*/
...
BEGIN
        ...
        BEGIN --INICIO-CUERPO PROCEDIMIENTO
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    SELECT @V_ERR_MENSAJE_USUARIO = 'ERROR AL EJECUTAR LA CONSULTA DE LA TABLA TBL_SIMP_PY_ACTIVIDAD';

				SELECT	@SET_SQLSTRING =
				N'SELECT [PK_TBL_SIMP_PY_ACTIVIDAD]
					  ,[FK_TBL_SIMP_PY_FASE]
					  ,[FK_TBL_SIMP_USUARIO]
                      ,[FK_TBL_SIMP_ESTADO]
					  ,[DESCRIPCION]
					  ...
					  ,[FECHA_ESTIMADA]
					  ,[FECHA_FINALIZACION]
				  FROM ' + @P_ESQUEMA + N'.[TBL_SIMP_PY_ACTIVIDAD]
				  ORDER BY PK_TBL_SIMP_PY_ACTIVIDAD asc'
				  ...
				  SELECT @PARAM_DEFINITION
					= '@P_ID bigint,
					   ...
					   @P_FECHA_ESTIMADA DATETIME,
                       @P_FECHA_FINALIZACION DATETIME,
				...
				N'SELECT [PK_TBL_SIMP_PY_ACTIVIDAD]
					  ...
					  ,[FECHA_ESTIMADA]
					  ,[FECHA_FINALIZACION]
				  FROM ' + @P_ESQUEMA + N'.[TBL_SIMP_PY_ACTIVIDAD] WHERE  ([PK_TBL_SIMP_PY_ACTIVIDAD] =  @P_ID OR @P_ID = 0) 
				  AND ([FK_TBL_SIMP_PY_FASE] = @P_IDFASE OR @P_IDFASE = 0)
				  AND ([FK_TBL_SIMP_USUARIO] = @P_IDUSUARIO OR @P_IDUSUARIO = 0)
				  AND ([FK_TBL_SIMP_ESTADO] = @P_IDESTADO OR @P_IDESTADO = 0)
				  AND ([DESCRIPCION] = @P_DESCRIPCION OR @P_DESCRIPCION = '''')
				  AND ([FECHA_INICIO] = @P_FECHA_INICIO OR @P_FECHA_INICIO = '''')
				  AND ([FECHA_ESTIMADA] = @P_FECHA_ESTIMADA OR @P_FECHA_ESTIMADA = '''')
                  AND ([FECHA_FINALIZACION] = @P_FECHA_FINALIZACION OR @P_FECHA_FINALIZACION = '''')'
    ...
    BEGIN CATCH
       ...
       SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, '@P_OPCION=', ISNULL(@P_OPCION, -1));
        ...
        SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_ID=', '''', ISNULL(@P_ID, -1));
        SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_IDFASE=', '''', ISNULL(@P_IDFASE, -1));
		SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_IDUSUARIO=', '''', ISNULL(@P_IDUSUARIO, -1));
        SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_IDESTADO=', '''', ISNULL(@P_IDESTADO, -1));
		SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_DESCRIPCION=', '''', ISNULL(@P_DESCRIPCION, 'N/D'),'''');
		SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_FECHA_INICIO=', '''', ISNULL(@P_FECHA_INICIO, 'N/D'), '''');
		SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_FECHA_ESTIMADA=', '''', ISNULL(@P_FECHA_ESTIMADA, 'N/D'), '''');
        SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_FECHA_FINALIZACION=', '''', ISNULL(@P_FECHA_FINALIZACION, 'N/D'), '''');
