...
FECHA:          <2022/09/10>
DESCRIPCION:   <SELECCIONA PROYECTOS>
-------------------------------------------

=========================================COMPLETAR CON NUEVOS PARAMETROS============================================================================

EXEC dbo.PA_CON_PROYECTOS	@P_USUARIO = '',            -- varchar(20)
                                    @P_OPCION = 0,              -- smallint
                                    @P_ID = 0,					-- int
                                    @P_ESTADO = '',             -- char(1)
...
*/
GO
CREATE PROCEDURE [dbo].[PA_CON_PROYECTOS]
(
     ...
     @P_ID BIGINT = 0,								/*ID DEL USUARIO */
     @P_IDCLIENTE BIGINT = 0,						/*FK DE LA TABLA CLIENTE*/
	 @P_IDESTADO BIGINT = 0,						/*FK DE LA TABLA ESTADO*/
	 @P_NOMBRE VARCHAR(50) ='',						/*NOMBRE DEL USUARIO*/
	 @P_DESCRIPCION VARCHAR(250) = '',			    /*DESCRIPCION DEL PROYECTO*/
	 @P_FECHA_INICIO DATETIME,			            /*FECHA INICIO PROYECTO*/
	 @P_FECHA_ESTIMADA DATETIME,					/*FECHA ESTIMADA DE FINALIZACION DEL PROYECTO*/
     @P_FECHA_FINALIZACION DATETIME,				/*FECHA REAL DE FINALIZACION DEL PROYECTO*/
...
BEGIN
        ...
        BEGIN --INICIO-CUERPO PROCEDIMIENTO
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    SELECT @V_ERR_MENSAJE_USUARIO = 'ERROR AL EJECUTAR LA CONSULTA DE LA TABLA TBL_SIMP_PY_PROYECTO';

				SELECT	@SET_SQLSTRING =
				N'SELECT [PK_TBL_SIMP_PY_PROYECTO]
					  ,[FK_TBL_SIMP_CLIENTE]
					  ,[FK_TBL_SIMP_ESTADO]
					  ,[NOMBRE]
					  ,[DESCRIPCION]
					  ...
					  ,[FECHA_ESTIMADA]
					  ,[FECHA_FINALIZACION]
				  FROM ' + @P_ESQUEMA + N'.[TBL_SIMP_PY_PROYECTO]
				  ORDER BY PK_TBL_SIMP_PY_PROYECTO asc'
				...
				N'SELECT [PK_TBL_SIMP_PY_PROYECTO]
					  ...
					  ,[FECHA_ESTIMADA]
					  ,[FECHA_FINALIZACION]
				  FROM ' + @P_ESQUEMA + N'.[TBL_SIMP_PY_PROYECTO] WHERE  ([PK_TBL_SIMP_PY_PROYECTO] =  @P_ID OR @P_ID = 0) 
				  AND ([FK_TBL_SIMP_CLIENTE] = @P_IDCLIENTE OR @P_IDCLIENTE = 0)
    ...
    BEGIN CATCH
       ...
       SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, '@P_OPCION=', ISNULL(@P_OPCION, -1));
        ...
        SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_IDCLIENTE=', '''', ISNULL(@P_IDCLIENTE, -1));
		SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_IDESTADO=', '''', ISNULL(@P_IDESTADO, -1));
		SELECT @V_ERR_PARAMETROS = CONCAT(@V_ERR_PARAMETROS, ',@P_NOMBRE=', '''', ISNULL(@P_NOMBRE, 'N/D'),'''');
