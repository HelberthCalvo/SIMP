...
(
    ...
    @P_ESQUEMA VARCHAR(100) = ''					/*COMPAÑIA*/
)
AS
BEGIN
    ...
    BEGIN TRY
            ...
            BEGIN --INICIO-VARIOS
                ...
                SELECT @V_ERR_PARAMETROS
                    = (
                                ...
                                ' , '+
                                '@P_IDFASE='+
                                CAST(@P_IDFASE AS VARCHAR(50))+
                                ' , '+
                                '@P_IDUSUARIO='+
                                CAST(@P_IDUSUARIO AS VARCHAR(50))+
                                ' , '+
                                '@P_IDESTADO ='+
                                CAST(@P_IDESTADO AS VARCHAR(50))+
                                ' , '+
                                '@P_DESCRIPCION='+
                                ''''+
                                @P_DESCRIPCION+
                                ''''+
                                '@P_FECHA_INICIO ='+
                                CAST(@P_FECHA_INICIO AS VARCHAR(50))+
                                ' , '+
                                '@P_FECHA_ESTIMADA ='+
                                CAST(@P_FECHA_ESTIMADA AS VARCHAR(50))+
                                ' , '+
        ...
        BEGIN --INICIO-CUERPO PROCEDIMIENTO								   
            ...
            BEGIN
                ...
                SELECT @V_ERR_MENSAJE_USUARIO = 'ERROR AL SELECCIONAR EN LA TABLA TBL_SIMP_PY_ACTIVIDAD';
			    ...
			    IF NOT EXISTS(SELECT 1 FROM  ' + @P_ESQUEMA
                          + '.TBL_SIMP_PY_ACTIVIDAD
				 WHERE PK_TBL_SIMP_PY_ACTIVIDAD = @P_ID)
				 BEGIN
				 ...
				 (   
				  ...
				  BEGIN
				  UPDATE  ' + @P_ESQUEMA
							+ '.TBL_SIMP_PY_ACTIVIDAD SET 
					 FK_TBL_SIMP_PY_FASE =  @P_IDFASE,
					 FK_TBL_SIMP_USUARIO = @P_IDUSUARIO,
					 FK_TBL_SIMP_ESTADO = @P_IDESTADO,
					 DESCRIPCION = @P_DESCRIPCION
                     FECHA_INICIO =  @P_FECHA_INICIO,
					 FECHA_ESTIMADA = @P_FECHA_ESTIMADA,
					 FECHA_FINALIZACION = @P_FECHA_FINALIZACION
					 WHERE PK_TBL_SIMP_PY_ACTIVIDAD = @P_ID
				  END						
						';

                    SELECT @V_ERR_NUMERO_USUARIO = 75001; /*INCREMENTAR PARA CADA ACCIÓN*/
                    ...
                    SELECT @V_ERR_NUMERO_USUARIO = 75002; /*INCREMENTAR PARA CADA ACCIÓN*/
